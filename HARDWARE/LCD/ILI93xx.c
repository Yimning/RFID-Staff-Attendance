...
{ 										  
	...
	//FSMC_Bank1E的地址为0xA00000104，实际为FSMC_BWTR1的地址
...
void LCD_Clear1(u16 color1,u16 color2)
{
	u32 index=0;      
	u32 totalpoint=lcddev.width;
	totalpoint*=lcddev.height; 			//得到总点数
	...
	LCD_WriteRAM_Prepare();     		//开始写入GRAM	 	  
	for(index=0;index<totalpoint/2;index++)
	{
		LCD->LCD_RAM=color1;	
	}
	for(index=totalpoint/2;index<totalpoint;index++)
	{
		LCD->LCD_RAM=color2;	
	}
} 
